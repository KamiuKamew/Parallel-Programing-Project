...
- [并行程序设计实验报告](#并行程序设计实验报告)
  ...
  - [性能分析](#性能分析)
    - [各版本的测试结果](#各版本的测试结果)
    - [使用 perf 进行深入剖析](#使用-perf-进行深入剖析)
      - [逻辑复杂度](#逻辑复杂度)
      - [内部数据依赖导致流水线阻塞](#内部数据依赖导致流水线阻塞)
      - [结论](#结论)
...
因此，我打算打乱顺序，按照“基础串行代码——便于并行化的串行代码——封装基本并行运算——并行化代码”的顺序撰写。最后，我们统一分析各个步骤带来的性能提升，以及使用 perf 工具进行深入剖析。
...
以下是各个版本在不同规模下的平均延迟（单位：微秒）：

| 实现版本                 | n=4     | n=131072 | 对大规模的加速比 |
| ------------------------ | ------- | -------- | ---------------- |
| 基础串行（naive）        | 0.00018 | 95420.7  | 1.0×             |
| 优化串行（serial）       | 0.0041  | 104.736  | 910.9×           |
| Montgomery（montgomery） | 0.00485 | 76.1702  | 1252.7×          |
| SIMD 并行（parallel）    | 0.01747 | 65.9827  | 1446.1×          |

所有测试均在校方服务器环境下进行。

在对数坐标下，可以直观地对比各个版本在不同数据规模下的平均延迟：

![alt text](image/2.png)

我们可以发现，在小规模数据（n=4）下，基础串行版本反而表现出最低的延迟。我们认为，这是因为优化版本引入的额外开销（如初始化 Montgomery 常量、SIMD 向量的包装与解包等）在小数据量下无法抵消获得的收益；甚至，在`n = 4`的极小规模下，我们的代码完全无法进行 SIMD 运算，反而由于向量化和去向量化的操作引入了大量的无用操作。
...
我们使用 perf 记录我们的并行优化代码对长为 32768 的多项式进行乘法运算的过程，并使用 perf 的 report 工具查看结果。下面是部分摘录与分析：

| Children | Self       | Symbol                                    | 分析与注释                                               |
| :------- | :--------- | :---------------------------------------- | :------------------------------------------------------- |
| 99.64%   | 0.00%      | `_start` -> `__libc_start_main` -> `main` | 主程序（包括随机测试样例生成）。                         |
| 89.13%   | 0.36%      | `poly_multiply_ntt_simd`                  | 多项式乘法。                                             |
| 47.64%   | 1.81%      | `ntt_forward_mont_simd`                   | 正向 NTT。                                               |
| 42.93%   | **42.93%** | `MontModNeon::reduce_pair`                | 热点之一：Montgomery reduction 双输入版（reduce_pair）。 |
| 25.36%   | 0.91%      | `ntt_inverse_dit_mont_simd`               | 逆向 NTT。                                               |
| 24.46%   | **24.46%** | `MontModNeon::mul`                        | 热点之二：SIMD 版的模乘。                                |
| 8.51%    | 8.51%      | `bit_reverse_permute`                     | 位反转置换。                                             |
| 5.43%    | 5.43%      | `MontMod::reduce`                         | 标准版的 Montgomery reduction。                          |
| 5.07%    | 5.07%      | `MontModNeon::sub`                        | SIMD 的模减。                                            |
| 3.99%    | 3.99%      | `MontModNeon::add`                        | SIMD 的模加。                                            |

也可参照火焰图：

![alt text](image/3.svg)

可以看到，`MontModNeon::reduce_pair`和`MontModNeon::mul`所占用的运行时间是最多的。

我们不难理解`MontModNeon::mul`占用时间较长（因为多项式乘法本身便会用到大量的乘法）。事实上，由于`MontModNeon::mul`会调用`MontModNeon::reduce_pair`，后者占用时间较长也不难理解。

下面我们分析`MontModNeon::reduce_pair`占用时间长的原因：

#### 逻辑复杂度

在 `reduce_pair` 里，实际执行的操作很多：

- `vmovn_u64`：64 位降到 32 位
- `vmul_u32`：乘法
- `vmlal_u32`：64 位加乘（multiply-accumulate）
- `vshrn_n_u64`：右移
- `vcgeq_u32`：比较
- `vandq_u32`：按位与
- `vsubq_u32`：减法

不仅如此，其中，`vmlal_u32`和`vshrn_n_u64`耗时都较长。

另一方面，`vcgeq_u32`、`vandq_u32`和`vsubq_u32`构成逻辑判断，使依赖关系变得复杂，产生了内部数据依赖，导致流水线阻塞。

#### 内部数据依赖导致流水线阻塞

从指令链看，`reduce_pair`是顺序依赖非常密集的：

- 要先 `vmovn`（截断低 32 位），才能 `vmul`（乘 neg_r_inv）
- 要 `vmlal`（t + m×mod），然后才能 `vshrn`（右移）
- 要 `vshrn` 之后才能做比较、条件修正

因为没有办法把这些阶段并行展开，所以 CPU 在执行这段的时候，流水线容易空转等待。

#### 结论

`MontModNeon::reduce_pair` 之所以占用这么多时间，既有逻辑复杂度高的原因，也有数据强依赖导致的流水线阻塞的原因。
