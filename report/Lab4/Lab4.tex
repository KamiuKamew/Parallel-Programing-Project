...
\texttt{thread\_overhead.h}通过对比传统的重复创建线程版本和线程池复用版本的执行时间，测量线程创建和销毁操作带来的性能开销。在传统版本中，我们使用标准的OpenMP并行for循环，每次计算都会\textbf{创建和销毁线程}。线程池复用版本通过手动维护线程池规避了大量的线程创建与销毁。
...
我们实现了多种内存访问模式的性能测试，包括顺序访问、随机访问和伪共享测试。该测试通过对比理论内存带宽和实际达到的带宽来评估并行效率。
...
private:
    ...
    double measure_false_sharing_overhead(u64 n, int num_threads) {
        ...
        // 测试无伪共享的版本
        u64* separate_data = new u64[num_threads * 64];  // 每个线程64个u64，避免伪共享
...
这种对齐方式确保了数据结构按照CPU缓存行边界对齐，有效避免了多线程访问时的伪共享问题。
