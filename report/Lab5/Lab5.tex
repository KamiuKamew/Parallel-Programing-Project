...
\begin{abstract}
多项式乘法是同态加密、数字信号处理等领域的核心算子，而数论变换（NTT）是将其复杂度从 $O(n^2)$ 降至 $O(n\log n)$ 的关键算法。本报告系统梳理了 SIMD、OpenMP、MPI、CUDA 四级并行优化的完整实验过程，并最终构建了一个统一框架，支持三种模运算实现（朴素、Montgomery、Barrett）与四种并行策略（串行、OpenMP、MPI、GPU）的任意组合。

在过去的实验中，我们在 SIMD 实验中实现了 ARM NEON 向量化与 Montgomery 规约，为后续并行化奠定了高效标量运算基础。随后，OpenMP 多线程实验在中等规模数据上实现 2–3 倍加速；MPI 实验则验证了 CRT 划分在分布式场景下的可行性，为 u64 精度扩展提供了设计样板；GPU 实验通过"线程级蝶形 + 共享内存 + 预计算旋转因子"把运行时间压缩到十微秒级，在 $n=131072$ 时较串行快 6.5 倍。
...
尾声部分回顾了各实验的技术收获与遗憾，并展望了未来方向。实验结果证明，面向硬件特性与算法结构的协同优化，能为高性能多项式乘法提供数量级的性能提升。
\end{abstract}
% 生成目录
\clearpage
\tableofcontents
...
回顾整个 NTT 并行优化系列实验，我们历时一个学期，先后完成了 SIMD 指令级并行（Lab2）、多线程并行（Lab3）、多进程分布式并行（Lab4）、GPU 极细粒度并行（Lab5）以及最后的统一框架与全面性能测试。一路走来，我们既经历了“写出来能跑”的朴素喜悦，也经历了在缓存失效、线程争用与通信瓶颈面前的各种失败。

\paragraph{SIMD 实验的收获与遗憾} 在 Lab2 中，我们第一次将 NTT 的蝶形循环拆解为向量化友好的形态，尝试使用 ARM NEON 指令集实现四路并行的 Montgomery 模运算。通过重构数据布局、预计算常量、手写汇编级 intrinsic，我们让性能提升了三个数量级，也第一次体会到“把数学写进寄存器”的快感。然而，由于实验服务器并不支持 NEON，最终的性能只能通过离线跑分间接验证，这成为本系列实验最早的遗憾：缺乏真正的硬件环境，很难进一步探索更深层的 SIMD 优化，例如混合基 NTT 与向量内分段卷积。

\paragraph{多线程实验的收获与遗憾} Lab3 让我们直面 CPU 多核并行的工程细节。我们学会了用数据依赖和任务粒度来定位并行位点，实现了线程池，体会到如何用条件变量与原子计数来平衡开销与吞吐。这一阶段最大的收获是建立了“串行代码 $\rightarrow$ 便于并行化的重构 $\rightarrow$ 并行代码”的范式。但也正是在这一阶段，我们感受到被内存带宽的上限所限制的加速极限：四个物理核心在 8 线程以后几乎不再加速，说明计算已经主要由带宽限制。

\paragraph{MPI 实验的收获与遗憾} Lab4 从共享内存转向了分布式内存。为了适配64比特整数的大模数测试点，我们使用了中国剩余定理把同一段计算复制了四遍，这一操作使把理论上的四进程加速变成了四倍计算量，完全抵消了MPI带来的加速效应。

\paragraph{GPU 实验的收获与遗憾} 我们把三层循环展平成“一线程一蝶形”，用预计算旋转因子消除了循环携带依赖，再用共享内存消化数据复用，第一次让大量 thread 在同一个 kernel 中同时运算。遗憾同样存在：由于时间仓促，我们不仅没有达到预期性能，还少完成了两个模乘的实现。不过，我们在本次实验中弥补了这一遗憾，我们不仅实现了所有的三个模乘、对比了它们的性能，还把加速比从不到2提升到了4-5.

\paragraph{统一框架与测试的反思} 最终的统一框架把三种模运算与四种并行策略装进同一个函数接口。真正运行大规模测试时，我们才发现：最耗时的不是计算，而是把所有结果汇总、对齐、可视化——这让我们意识到工程化的重要性。

短暂的一个学期无法穷尽 NTT 优化的全部可能，但从第一次把蝶形运算写成循环，到今天能在 GPU 上跑到十微秒级延迟，我们已经完成了从“写对”到“写快”再到“写稳”的渐进旅程。实验会结束，但分析依旧、改进亦然。愿这些代码片段、火焰图和性能曲线，能成为下一次优化的起点。
