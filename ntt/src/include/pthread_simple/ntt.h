...
inline void poly_multiply_ntt_pthread_simple(T *a, T *b, T *ab, T n, T p, T OMEGA = 3)
{
  ...
  ntt_forward_mont_pthread_simple(a_mont, n_expanded, p, omega_mont);
  ntt_forward_mont_pthread_simple(b_mont, n_expanded, p, omega_mont);

  for (T i = 0; i < n_expanded; ++i)
    ab_mont[i] = montMod.mul(a_mont[i], b_mont[i]);

  ntt_inverse_mont_pthread_simple(ab_mont, n_expanded, p, montMod.inv(omega_mont));
