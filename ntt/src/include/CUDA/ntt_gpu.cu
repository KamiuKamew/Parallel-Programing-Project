...
template <typename T>
__device__ T gpu_mont_pow_correct(T base_mont, T exp, T mod, T neg_r_inv, T one_mont) {
    T result = one_mont;
    while (exp > 0) {
        if (exp & 1) {
...
// 朴素模乘
template <typename T>
__device__ inline T mod_mul_naive(T a, T b, T p) {
    return (T)(((__uint128_t)a * b) % p);
}

// Barrett规约（仅32位）
__device__ inline u32 mod_mul_barrett_32(u32 a, u32 b, u32 p, u64 mu) {
    u64 z = (u64)a * b;
    u64 q = (z * mu) >> 32;
    u32 r = (u32)(z - q * p);
    return (r >= p) ? (r - p) : r;
...
template <typename T, int MODE>
__global__ void ntt_stage_kernel_inv_mode(T *a, const T *twiddles, int mid, int n, T p, T neg_r_inv, u64 mu) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int total = n >> 1;
    if (tid >= total) return;
...
template <typename T>
__global__ void pointwise_multiply_kernel_correct(T *a_mont, T *b_mont, T *ab_mont, T n, T mod, T neg_r_inv) {
    T idx = blockDim.x * blockIdx.x + threadIdx.x;
    if (idx < n) {
        ab_mont[idx] = gpu_mont_mul_correct(a_mont[idx], b_mont[idx], mod, neg_r_inv);
    }
}
...
void poly_multiply_ntt_gpu_core(T *a, T *b, T *ab, T n, T p, T omega) {
    ...
    CHECK_CUDA_CORRECT(cudaMemcpy(a_mont_gpu, a_mont_host, n_expanded * sizeof(T), cudaMemcpyHostToDevice));
    CHECK_CUDA_CORRECT(cudaMemcpy(b_mont_gpu, b_mont_host, n_expanded * sizeof(T), cudaMemcpyHostToDevice));
    ...
    for (int level = (int)std::log2(n_expanded) - 1; level >= 0; --level) {
        ...
        ntt_stage_kernel_inv_mode<T, MODE><<<GRID, BLOCK>>>(ab_mont_gpu, tw_inv_gpu, mid, n_expanded, p, neg_r_inv, mu);
        CHECK_CUDA_CORRECT(cudaGetLastError());
    }
    CHECK_CUDA_CORRECT(cudaDeviceSynchronize());
    ...
    CHECK_CUDA_CORRECT(cudaMemcpy(temp_result, ab_mont_gpu, n_expanded * sizeof(T), cudaMemcpyDeviceToHost));
...
// 显式实例化
template void poly_multiply_ntt_gpu_naive<u32>(u32 *a, u32 *b, u32 *ab, u32 n, u32 p, u32 omega);
template void poly_multiply_ntt_gpu_naive<u64>(u64 *a, u64 *b, u64 *ab, u64 n, u64 p, u64 omega);
template void poly_multiply_ntt_gpu_mont<u32>(u32 *a, u32 *b, u32 *ab, u32 n, u32 p, u32 omega);
template void poly_multiply_ntt_gpu_mont<u64>(u64 *a, u64 *b, u64 *ab, u64 n, u64 p, u64 omega);
template void poly_multiply_ntt_gpu_barrett<u32>(u32 *a, u32 *b, u32 *ab, u32 n, u32 p, u32 omega);
template void poly_multiply_ntt_gpu_barrett<u64>(u64 *a, u64 *b, u64 *ab, u64 n, u64 p, u64 omega);
template void poly_multiply_ntt_gpu<u32>(u32 *a, u32 *b, u32 *ab, u32 n, u32 p, u32 omega);
template void poly_multiply_ntt_gpu<u64>(u64 *a, u64 *b, u64 *ab, u64 n, u64 p, u64 omega); 