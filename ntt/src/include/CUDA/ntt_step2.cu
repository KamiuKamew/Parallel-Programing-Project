...
// CUDA错误检查辅助函数
void check_cuda_error_step2(const char *msg) {
...
// 第二步改进：预处理所有旋转因子
void precompute_twiddle_factors_step2(u64 **d_twiddles, u64 n, u64 p,
                                      u64 omega_mont) {
  ...
  for (u64 mid = 1; mid < n; mid <<= 1) {
    u64 exp = (p - 1) / (mid << 1);
    // 修复BUG：应该使用传入的omega_mont，不是固定的3
  ...
  check_cuda_error_step2("precompute twiddle factors");
...
// 第二步改进：重构的NTT正变换kernel（统一处理，便于CUDA并行化）
__global__ void ntt_forward_step2_kernel(u64 *a_mont, u64 n, u64 p,
                                         u64 *d_twiddles, u64 neg_r_inv) {
  if (threadIdx.x == 0 && blockIdx.x == 0) {
    u64 twiddle_offset = 0;

    for (u64 mid = 1; mid < n; mid <<= 1) {
      // 统一处理：每个mid层级都有n/2个独立的蝶形运算
      // 第三步将简单地将这个循环替换为GPU线程
      for (u64 tid = 0; tid < n / 2; ++tid) {
        // 从线程ID映射到(j,k)坐标
...
// 第二步改进：重构的NTT逆变换kernel（统一处理，便于CUDA并行化）
__global__ void ntt_inverse_step2_kernel(u64 *a_mont, u64 n, u64 p,
...
// 第二步改进：逐点乘法（统一处理，便于CUDA并行化）
__global__ void pointwise_mul_step2_kernel(u64 *a_mont, u64 *b_mont,
...
void poly_multiply_ntt_cuda_parallel_ready(u64 *a, u64 *b, u64 *ab, u64 n,
  ...
  precompute_twiddle_factors_step2(&d_twiddles, n_expanded, p, omega_mont);
  precompute_twiddle_factors_step2(&d_twiddles_inv, n_expanded, p,
                                   omega_inv_mont);
  ...
  check_cuda_error_step2("GPU memory allocation");
  ...
  cudaMemcpy(d_b_mont, b_mont_cpu, n_expanded * sizeof(u64),
             cudaMemcpyHostToDevice);
  check_cuda_error_step2("copy to GPU");

  // 第二步改进：使用重构的kernel
  ntt_forward_step2_kernel<<<1, 1>>>(d_a_mont, n_expanded, p, d_twiddles,
                                     neg_r_inv);
  cudaDeviceSynchronize();
  check_cuda_error_step2("ntt_forward_step2_kernel");

  ntt_forward_step2_kernel<<<1, 1>>>(d_b_mont, n_expanded, p, d_twiddles,
                                     neg_r_inv);
  cudaDeviceSynchronize();
  check_cuda_error_step2("ntt_forward_step2_kernel");

  // 逐点乘法
  pointwise_mul_step2_kernel<<<1, 1>>>(d_a_mont, d_b_mont, d_ab_mont,
                                       n_expanded, p, neg_r_inv);
  cudaDeviceSynchronize();
  check_cuda_error_step2("pointwise_mul_step2_kernel");

  // 第二步改进：使用重构的逆变换kernel
  ntt_inverse_step2_kernel<<<1, 1>>>(d_ab_mont, n_expanded, p, d_twiddles_inv,
                                     neg_r_inv, r2);
  cudaDeviceSynchronize();
  check_cuda_error_step2("ntt_inverse_step2_kernel");

  // 结果处理（与第一步相同）
  u64 *result_mont_cpu = new u64[n_expanded];
  cudaMemcpy(result_mont_cpu, d_ab_mont, n_expanded * sizeof(u64),
             cudaMemcpyDeviceToHost);
  check_cuda_error_step2("copy to CPU");
