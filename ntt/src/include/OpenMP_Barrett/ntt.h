 ...
 */
template <typename T>
inline void ntt_inverse_omp_Barrett(T *a, T n, T p, T omega)
{
    BarrettMod<T> mod(p, 100);
 ...
 */
template <typename T>
inline void poly_multiply_ntt_omp_Barrett(T *a, T *b, T *ab, T n, T p, T OMEGA = 3)
{
    BarrettMod<T> mod(p, 100);
    ...
    ntt_forward_omp_Barrett(a_copy, n_expanded, p, OMEGA);
    ntt_forward_omp_Barrett(b_copy, n_expanded, p, OMEGA);

    for (T i = 0; i < n_expanded; ++i)
        ab_result[i] = mod.mul(a_copy[i], b_copy[i]);

    ntt_inverse_omp_Barrett(ab_result, n_expanded, p, mod.inv(OMEGA));
